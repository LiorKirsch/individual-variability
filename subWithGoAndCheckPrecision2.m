function subWithGoAndCheckPrecision2()
...
%     [meanPerPerson, stdPerPerson] = calcNormalForEachPerson(randomSizePrecision.percentInCategory);
   ...
   % drawPrecisionStatistics(meanPerPerson, stdPerPerson, randomSizePrecision.sizesOfClasses);
    ...
    regionScores = nan(numberOfRegions,1);
    regionStdScores = nan(numberOfRegions,1);
    for m =1:numberOfRegions
        ...
        categoriesScores = clusteringScore(currentAreaDataMatrix, currentSubjectMatrix);
        regionScores(m) = mean(categoriesScores);
        regionStdScores(m) = std(categoriesScores);
    end
    
    regionColors = createColorMap(length(regionNames));
    figure(1234123);
    drawBars(regionScores,regionStdScores,regionNames,regionColors,'within / between');
    ...
    for m =1:numberOfRegions
        ...
        for i = 1:size(catDistanceScore,1)
            ...
            categoryDataMatrix = currentAreaDataMatrix(:,currentCategorySubset);
            ...
            catDistanceScore(i,:,m) = clusteringScore(categoryDataMatrix, currentAreaSamplesSubjectLabels);
            printPercentCounter(i, length(cat_ids) );
        end
    end 
   save('catScoresNotRemove.mat', 'catDistanceScore', 'cat_ids', 'go_gene_mat','geneNames','experimentsDataMatrix','numOfGenesInCategory','regionNames','experimentRegion');
