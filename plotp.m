...
sigms2 = [sigm-2*sigs fliplr(sigm+2*sigs)];
...
fill(time2(~isnan(sigms)), sigms(~isnan(sigms)),color1,'EdgeColor',color1,'FaceAlpha', 0.4);
fill(time2(~isnan(sigms2)), sigms2(~isnan(sigms2)),color2,'EdgeColor',color2,'FaceAlpha', 0.3);
% fill(time2(~isnan(sigmm)), sigmm(~isnan(sigmm)),color2,'EdgeColor',color2,'FaceAlpha', 0.3);
legend([{'Mean'}  {'\pm Stddev'} {'\pm 2 * Stddev'} ],'Location','Best')